...
# --- Fetch fixtures (next 30 days) ---
fixtures_url = f"{BASE_URL}fixtures?league={league_id}&season={season}&from={date_from}&to={date_to}"
fixtures_resp = requests.get(fixtures_url, headers=HEADERS).json()

# --- Fallback to past 30 days if no upcoming fixtures ---
if "response" not in fixtures_resp or not fixtures_resp["response"]:
    ...
    past_to = today.strftime("%Y-%m-%d")
    fallback_url = f"{BASE_URL}fixtures?league={league_id}&season={season}&from={past_from}&to={past_to}"
    fixtures_resp = requests.get(fallback_url, headers=HEADERS).json()

# --- Stop if still no fixtures ---
if "response" not in fixtures_resp or not fixtures_resp["response"]:
    st.error("❌ No fixtures found. Please try a different league or season.")
    st.stop()

fixtures = fixtures_resp["response"]
